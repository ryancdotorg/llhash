...
static void sha1_xform_invalid(uint32_t *a, const void *b, uint32_t c) {
  raise(SIGILL);
}
#pragma GCC diagnostic pop

static void sha1_xform_default(uint32_t *a, const void *b, uint32_t c) {
...
void (*SHA1_Transform)(uint32_t *, const void *, uint32_t) = sha1_xform_default;

#define MAYBE_REGISTER(impl,IMPL) \
  extern int JOIN(can,impl)(); \
  extern int JOIN(sha1,impl,xform,built)(); \
  extern void JOIN(sha1,impl,xform)(uint32_t *, const void *, uint32_t); \
  if (enable & JOIN(SHA1_ENABLE,IMPL)) { \
    int e = 0; \
    if (!JOIN(sha1,impl,xform,built)()) e |= 1; \
